// Envoie une invitation par email pour un collaborateur d'agenda, via le système natif Supabase
// Auth (admin.inviteUserByEmail) — même logique que invite-collaborator (project_collaborators),
// mais indexée par owner_id (le compte propriétaire de l'agenda) plutôt que project_id. Doit être
// appelée avec le token de l'utilisateur qui invite (déjà fait automatiquement par
// supabase.functions.invoke côté client) ; vérifie elle-même que cet utilisateur a bien un accès
// en écriture à l'agenda concerné avant d'envoyer quoi que ce soit.
...
Deno.serve(async (req) => {
  ...
  try {
    const { owner_id, email, redirect_to } = await req.json();
    if (!owner_id || !email) {
      return new Response(JSON.stringify({ error: "owner_id et email requis" }), {
        status: 400,
    ...
    const { data: hasAccess, error: accessError } = await admin.rpc("has_agenda_access", {
      oid: owner_id,
      uid: userData.user.id,
      require_write: true,
    });
    if (accessError || !hasAccess) {
      return new Response(JSON.stringify({ error: "Accès refusé à cet agenda" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (inviteError) {
      ...
      //   n'invite que les nouveaux comptes), on relie juste la ligne en attente, elle verra
      //   l'agenda à sa prochaine connexion normale ;
      ...
      if (alreadyRegistered) {
        ...
        if (existingProfile) {
          ...
          if (neverConfirmed) {
            ...
            // agenda_collaborators qui le référencent (contrainte "on delete cascade" sur
            ...
            const { data: pendingRows } = await admin
              ...
              .select("owner_id, permission, invited_by")
              .eq("user_id", existingProfile.id);

            await admin.auth.admin.deleteUser(existingProfile.id);

            if (pendingRows && pendingRows.length > 0) {
              await admin.from("agenda_collaborators").insert(
                pendingRows.map((r) => ({
                  owner_id: r.owner_id,
