  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-3">
        {(linked ?? []).map((l, i) => (
          <div key={l.id} className="group flex items-center gap-2 rounded-full border border-border py-1 pl-1 pr-3">
      ...
      <p className="text-xs text-muted-foreground">
        Liste réutilisable sur tous tes projets, gérée dans Réglages → Personnes. Utilisée pour répartir le budget par
        personne et rattacher une dépense à quelqu'un en particulier.
      </p>
    </div>
  );
}
